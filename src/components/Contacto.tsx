import AnimateOnScroll from "@/components/AnimateOnScroll";

export default function Contacto() {
  return (
    <section
      id="contacto"
      className="scroll-mt-14 border-t border-[#2a2a4a] px-4 py-24"
    >
      <AnimateOnScroll>
      <div className="mx-auto max-w-4xl text-center">
        <h2 className="mb-2 text-3xl font-bold sm:text-4xl">
          <span data-text="Contáctame" className="glitch-text neon-title flicker-erratic glow-magenta">
            Contáctame
          </span>
        </h2>
        <p className="mb-10 text-[#8888aa]">
          Hagamos realidad tu proyecto
        </p>

        <a
          href="https://wa.me/56982864145"
          target="_blank"
          rel="noopener noreferrer"
          className="card-glass card-glass-action card-glow card-glass-pulse mb-10 inline-flex items-center gap-3 rounded-lg border border-[#2a2a4a] bg-black/30 backdrop-blur-sm px-8 py-4 text-lg font-bold text-[#00ffff] transition-all hover:border-[#ff00ff]/50"
        >
          <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
          </svg>
          Escríbeme por WhatsApp
        </a>

        <div className="card-glass card-glow mx-auto max-w-md rounded-lg border border-[#2a2a4a] bg-black/30 backdrop-blur-sm p-8">
          <h3 className="mb-4 text-lg font-bold text-[#00ffff]">
            O contáctame directamente
          </h3>
          <p className="mb-2 flex items-center justify-center gap-2 text-xs text-body">
            <svg className="h-4 w-4 text-[#ff00ff]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <rect x="2" y="4" width="20" height="16" rx="2" />
              <path d="M22 7l-10 7L2 7" />
            </svg>
            <a
              href="mailto:vizcosoe@gmail.com"
            className="text-[#ff00ff] transition-colors hover:text-[#00ffff]"
            >
              vizcosoe@gmail.com
            </a>
          </p>
          <p className="flex items-center justify-center gap-2 text-xs text-body">
            <svg className="h-4 w-4 text-[#00ffff]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M12 2a8 8 0 0 0-8 8c0 5 8 12 8 12s8-7 8-12a8 8 0 0 0-8-8z" />
              <circle cx="12" cy="10" r="3" />
            </svg>
            Talca, Región del Maule, Chile
          </p>
        </div>
      </div>
      </AnimateOnScroll>
    </section>
  );
}
